...
window.mr_variant.wp.checkUser = function(){
	...
	jQuery.ajax({
		...
		error: function(response) {
			 window.mr_variant.variantNotification('Error Checking User', 'circle-x', 'error', 7000);
		},
...
window.mr_variant.wp.editImage = function(image) {
	...
	file_frame.on( 'select', function() {
		...
		window.mr_variant.wp.saveImage(attachment.url, attachment.alt);
		
		//Tell variant a change has been made
		window.mr_variant.setUnsaved();
	});
 ...
 */
window.mr_variant.wp.load = function() {
	...
	jQuery.ajax({
    	...
    	error: function(response) {
			window.mr_variant.variantNotification('Error loading page.', 'circle-x', 'error', 7000);
    	},
    	success: function(response) {
			//console.log(response);
    		window.mr_variant.importState(response);
    	},
      	complete: function(){
	        window.mr_variant.wp.renderNavContainer(wp_data.current_page.header_layout);
	        window.mr_variant.wp.renderFooter(wp_data.current_page.footer_layout);
	        window.mr_variant.wp.showWelcomeModal();
 ...
 * Create a new page
 */
window.mr_variant.wp.newPage = function($pageTitle, $saveType = 'newpage', $json, $html){
	
	window.mr_variant.wp.checkUser();
	
  	window.mr_variant.startLoading('.viu');
  	...
  	if( 'newpage' == $saveType ){
		...
		jQuery.ajax({
			...
			error: function(response) {
            	window.mr_variant.variantNotification('Failed to create new page.  Try again.', 'circle-x', 'warning', 3000);
			},
			success: function(response) {
			    wp_data = response;
	      		window.mr_variant.wpNewPage(window.mr_variant.wp.newPageName);
			},
		    complete: function() {
		        window.mr_variant.stopLoading('.viu');
		    }
		});
 ...
 * @since v1.0.0
 */
window.mr_variant.wp.saveHTML = function($html) {
	...
	if( typeof $html !== typeof undefined && $html !== false ){
	    ...
	    jQuery.ajax({
	    	...
	    	error: function(response) {
	    		  window.mr_variant.variantNotification('Failed to save HTML.  Try saving again.', 'circle-x', 'error', 3000);
	    	},
	    	success: function(response) {
	    	    if(!response.match('variant_page_builder_save_html_success')) {
	    		      window.mr_variant.variantNotification('There was a problem saving HTML.  Try saving again.', 'circle-x', 'error', 3000);	  
	    		  }
	    	},
        complete: function(){
            window.mr_variant.stopLoading('.viu');
    	  ...
    	  console.log($html);
        window.mr_variant.variantNotification('Warning: Cannot Save HTML. Refresh Page', 'circle-x', 'warning', 3000);
    }
   
};

/**
 * window.mr_variant.wp.saveVariant
 * 
 ...
 * @since v1.0.0
 */
window.mr_variant.wp.saveVariant = function(json, saveType = 'manual') {
    //console.log(json);
    ...
    if( typeof json !== typeof undefined && json !== false && json !== '' ){
		...
		jQuery.ajax({
	    	...
	    	success: function(response) {
                ...
                window.mr_variant.variantNotification('Page '+(saveType === 'auto' ? 'Autosaved' : 'Saved'), 'circle-check', 'success', 3000);
                window.mr_variant.setSaved();
  	    		}
	            if(response.match('0')) {
	                window.mr_variant.variantNotification('Warning: Page Not saved', 'bug', 'warning', 3000);
	            }
	    	}
	    });
          ...
          window.mr_variant.variantNotification('Warning: Cannot '+(saveType === 'auto' ? 'autosave' : 'save')+' Variant. Please refresh page.', 'circle-x', 'error', 3000);
...
/**
 * window.mr_variant.wp.renderHeader
 * 
 ...
 */
window.mr_variant.wp.renderNavContainer = function($layout){
	
  window.mr_variant.startLoading('.nav-container');
	jQuery.ajax({
		type: "POST",
		url: wp_data.ajax_url,
		data: {
		...
		success: function(response) {
        	window.mr_variant.renderNavContainer(response);
		},
		complete: function(){
			//Fix nested WYSIWYG
...
window.mr_variant.wp.updateHeader = function($layout){
  ...
  window.mr_variant.startLoading('.nav-container');
	jQuery.ajax({
		...
		success: function(response) {
        window.mr_variant.variantNotification('Header Updated', 'circle-check', 'success', 3000);
		}
	});
	
	window.mr_variant.wp.renderNavContainer($layout);
	
};

/**
 * window.mr_variant.wp.renderFooter
 * 
 ...
 * @toDo check with James if this is actually needed
 */
window.mr_variant.wp.renderFooter = function($layout){
	
	window.mr_variant.startLoading( jQuery('.viu footer').length ? 'footer' : '.viu' );
	...
	jQuery.ajax({
		...
		success: function(response) {
			//console.log(response);
			window.mr_variant.renderFooter(response);
		}
	});
	
};

window.mr_variant.wp.updateFooter = function($layout){
  ...
  jQuery.ajax({
		...
		success: function(response) {
			window.mr_variant.variantNotification('Footer Updated', 'circle-check', 'success', 3000);
		}
	});
	
	window.mr_variant.wp.renderFooter($layout);
	
};

/**
 * window.mr_variant.wp.renderShortcode
 * 
 ...
 * @since v1.0.0
 */
window.mr_variant.wp.renderShortcode = function(shortcode, targetID){
	...
	jQuery.ajax({
	    ...
	    success: function(response) {
	        ...
	        window.mr_variant.renderShortCode(rendered);
	    }
	});
 ...
 */
window.mr_variant.wp.handleShortcodeBlock = function(data){
	jQuery(data).text('');
};
...
window.mr_variant.wp.showWelcomeModal = function(){
    if( !jQuery('.viu .main-container section').length && jQuery('.variant-welcome-modal').length ){
        window.mr_variant.showSectionsSelector();
...
window.mr_variant.wp.editLink = function(href, text, target, variantID){
...
window.mr_variant.wp.updateWpData = function($post_id, callback){
...
window.mr_variant.wp.updateOption = function(){
...
jQuery(window).load(function(){
	...
	jQuery('.wysiwyg-discard-changes').click(function(){
		...
		//window.mr_variant.variantNotification('Text Changes Discarded', 'bug', 'warning', 1500);
	...
	jQuery('.wysiwyg-save-changes').click(function(){
		...
		window.mr_variant.variantNotification('Text Updated', 'circle-check', 'success', 1500);
	...
	jQuery('body').on('click', '#wp-link-cancel, #wp-link-backdrop, #wp-link-close', function(event) {
		...
		window.mr_variant.variantNotification('Link Changes Discarded', 'bug', 'warning', 1500);
	});
	...
	jQuery('body').on('click', '.hide-text #wp-link-submit', function(event) {
		...
		window.mr_variant.variantNotification('Link Updated', 'circle-check', 'success', 1500);
	...
	jQuery('.variant-welcome-modal .vex').click(function(){
		window.mr_variant.wp.updateOption();
	});
